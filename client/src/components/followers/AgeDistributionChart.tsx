...
const data = [
  { age: "18-24", value: 22 },
  { age: "25-34", value: 38 }, // Peak demographic
  { age: "35-44", value: 18 },
  { age: "45-54", value: 12 },
  { age: "55-64", value: 7 },
  { age: "65+", value: 3 },
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-background/95 backdrop-blur-md border border-border/50 p-3 rounded-lg shadow-xl flex flex-col">
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Faixa {payload[0].payload.age}</span>
...
export function AgeDistributionChart({ delay = 0 }: { delay?: number }) {
  return (
          ...
          >
            ...
            >
              {data.map((entry, index) => (
                <Cell 
                  key={`cell-${index}`} 
                  fill={entry.age === "25-34" ? "var(--color-primary)" : "var(--color-primary-foreground)"} 
                  className={entry.age === "25-34" ? "drop-shadow-sm" : "opacity-30"}
                />
              ))}
              <LabelList 
                dataKey="value" 

    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-background font-sans text-foreground overflow-x-hidden relative selection:bg-amber-500/30">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <motion.div 
          ...
          className="absolute top-[-20%] left-[-10%] w-[800px] h-[800px] rounded-full bg-primary/10 blur-[150px]"
        />
        <motion.div 
          animate={{ scale: [1, 1.2, 1], opacity: [0.05, 0.1, 0.05] }}
          transition={{ duration: 15, repeat: Infinity }}
          className="absolute bottom-[-10%] right-[-10%] w-[600px] h-[600px] rounded-full bg-primary/5 blur-[150px]"
        />
      </div>
          ...
          <div className="p-8 md:p-12 rounded-[2.5rem] bg-white/[0.04] backdrop-blur-[30px] border border-white/[0.08] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.7)] flex flex-col items-center overflow-hidden">
            ...
            <div className="flex flex-col items-center mb-10 text-center">
              <div className="w-16 h-16 rounded-2xl bg-white/[0.05] border border-white/[0.1] flex items-center justify-center mb-6 relative overflow-hidden group">
                ...
                <img src={branding.logo} alt="Logo" className="w-9 h-9 object-contain relative z-10 brightness-110" />
              </div>
              ...
              <p className="text-primary-foreground/60 text-base italic font-['Crimson_Text'] tracking-wide">
                “{branding.motto}”
              </p>
            </div>
            ...
            <form onSubmit={handleAuth} className="w-full space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      ...
                      <input 
                        ...
                        required={isSignUp}
                        className="h-14 pl-12 w-full rounded-2xl bg-white/[0.03] border border-white/10 text-white placeholder:text-white/20 focus:bg-white/[0.06] focus:border-primary/40 transition-all outline-none"
                      />
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/10 group-focus-within:text-primary transition-colors" />
                      <input 
                        id="phone"
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/10 group-focus-within:text-primary transition-colors" />
                  <input 
                    id="email"
                    type="email"
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center ml-1">
                  ...
                  {!isSignUp && (
                    <button type="button" className="text-[10px] font-black text-primary/60 hover:text-primary transition-colors uppercase tracking-widest decoration-dotted underline-offset-4 hover:underline">
                      Esqueceu?
                    </button>
                  )}
                </div>
                <div className="relative group">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/10 group-focus-within:text-primary transition-colors" />
                  <input 
                    id="password"
                    ...
                    required
                    className="h-14 pl-12 pr-12 w-full rounded-2xl bg-white/[0.03] border border-white/10 text-white placeholder:text-white/20 focus:bg-white/[0.06] focus:border-primary/40 transition-all outline-none"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 p-1.5 text-white/20 hover:text-primary transition-colors z-10"
              ...
              <Button 
                type="submit" 
                disabled={loading}
                className="w-full h-15 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-black text-lg shadow-xl shadow-primary/10 hover:-translate-y-1 transition-all duration-300 mt-4 group"
              >
                {loading ? (
                  ...
                  <div className="flex items-center justify-center gap-2 uppercase tracking-widest">
                    <span>{isSignUp ? "Criar Conta" : branding.welcomeMessage}</span>
            ...
            <div className="mt-10 text-center w-full">
              ...
              >
                {isSignUp ? (
                  <>Já é um membro? <span className="text-primary">Faça login</span></>
                ) : (
                  <>Ainda não tem acesso? <span className="text-primary">Crie sua conta</span></>
                )}
              </button>
            </div>
          </div>
      ...
      {isSignUp && !loading && (
        <motion.div 
          ...
          animate={{ opacity: 1, y: 0 }}
          className="mt-4 p-4 rounded-xl bg-primary/5 border border-primary/20 max-w-[400px] text-center"
        >
      ...
      <div className="mb-8 mt-10 opacity-40 text-center">
        <p className="text-[10px] uppercase tracking-[0.4em] text-white">{branding.footerText}</p>
      </div>
    </div>
  );
}
